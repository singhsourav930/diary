...
import { setSelectedNodeAction, setOpenNodesAction, setNodesAction, setOutlineSelectedNodeAction } from './sidebar.slice';
...
const insertNodeInOrder = (newNode: SidebarNodeType, existingNodes: SidebarNodeType[] = []): SidebarNodeType[] => {
...
export const createNode = (node: SidebarNodeType) => {
  ...
  const { outlineSelectedNode, openNodes, nodes } = state.sidebar as SidebarStateType;
  const nodeId = outlineSelectedNode?.isFolder ? outlineSelectedNode?.id : outlineSelectedNode?.parentReference?.id;
  if (nodeId) {
    setOpenNodes([...new Set([...openNodes, nodeId])]);
    const updatedNodes = (nodesList: SidebarNodeType[], nodeId: string): SidebarNodeType[] => {
      return nodesList.map(nodeItem => {
        if (nodeItem.id === nodeId) {
          if (nodeItem?.isFolder) {
            const existingChildren = nodeItem.children || [];
            return { ...nodeItem, children: insertNodeInOrder(node, existingChildren) }
          }
        }
        if (nodeItem.children) {
          return { ...nodeItem, children: updatedNodes(nodeItem.children as SidebarNodeType[], nodeId) }
        }
        return nodeItem
      })
    }
...
const sortNodesByName = (nodesList: SidebarNodeType[]): SidebarNodeType[] => {
  ...
  const folders = nodesCopy.filter(n => n.isFolder).sort((a, b) => a.name.localeCompare(b.name));
  const files = nodesCopy.filter(n => !n.isFolder).sort((a, b) => a.name.localeCompare(b.name));
...
export const updateNode = (node: SidebarNodeType) => {
  ...
  const updatedNodes = ({ nodesList, nodeId, isTemporary = false, isRenaming = false }: { nodesList: SidebarNodeType[], nodeId: string, isTemporary?: boolean, isRenaming?: boolean }): SidebarNodeType[] => {
    const sortedNodes = (!isTemporary && !isRenaming) ? sortNodesByName(nodesList) : nodesList;
    return sortedNodes.map(nodeItem => {
      ...
      if (nodeItem.children) {
        return { ...nodeItem, children: updatedNodes({ nodesList: nodeItem.children as SidebarNodeType[], nodeId: nodeId, isTemporary, isRenaming }) }
      }
      return nodeItem
    })
  }

  const newNodes = updatedNodes({ nodesList: nodes, nodeId: node.id, isTemporary: node.isTemporary, isRenaming: node.isRenaming });
  setNodes(newNodes);

  if (node.isTemporary) {
    if (node.keyName === ACTIVE_KEY_OR_STATE.ESCAPE || (node.keyName === ACTIVE_KEY_OR_STATE.BLUR && !Boolean(node.name.trim()))) {
      const updatedNodes = (nodesList: SidebarNodeType[], nodeId: string): SidebarNodeType[] => {
        return nodesList.filter(nodeItem => nodeItem.id !== nodeId).map(nodeItem => {
          if (nodeItem.children) {
            return { ...nodeItem, children: updatedNodes(nodeItem.children as SidebarNodeType[], nodeId) }
          }
          return nodeItem
    ...
    } else if ((node.keyName === ACTIVE_KEY_OR_STATE.BLUR || node.keyName === ACTIVE_KEY_OR_STATE.ENTER) && Boolean(node.name.trim())) {
...
export const renameNode = () => {
  ...
  const { outlineSelectedNode, nodes } = state.sidebar as SidebarStateType;
  if (outlineSelectedNode) {
    const updateNodes = (nodesList: SidebarNodeType[], nodeId: string): SidebarNodeType[] => {
      return nodesList.map(nodeItem => {
        if (nodeItem.id === nodeId) {
          return { ...nodeItem, isRenaming: true }
        }
        if (nodeItem.children) {
          return { ...nodeItem, children: updateNodes(nodeItem.children as SidebarNodeType[], nodeId) };
        }
        return nodeItem;
      });
    };
...
export const handleOpenNodes = (id: string) => {
  ...
  const nodeId = id;
  const newOpenNodes = openNodes.includes(nodeId) ? openNodes.filter(nodeItemId => nodeItemId !== nodeId) : [...openNodes, nodeId];
  setOpenNodes(newOpenNodes);
}

export const getActiveLineNodes = (selectedNode: SidebarNodeType | null): string[] => {
...
export const getFlattenedNodes = (nodes: SidebarNodeType[], openNodes: string[], parentReferenceNode: SidebarNodeType | null): SidebarNodeType[] => {
  const results: SidebarNodeType[] = [];
  const flatter = (nodeList: SidebarNodeType[], parentNode: SidebarNodeType | null, depth: number) => {
    nodeList.forEach(node => {
      results.push({
        ...node,
